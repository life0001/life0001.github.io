			 ...
			 // 如果有滚动条
 		 	if ($(document).height() >= $(document).height() - $(window).height()) {
 		 		if (scrolltop < pos) {
 					 ...
 					 if (options.isGap) {
 						 scrolltop < 0 ? $this.css({'position': 'initial', 'padding': '0', 'top': 0}).css('background', '') : $this.css({'position': 'fixed', 'padding': options.pad, 'top': options.top}).css('background-color', options.bgcolor);
 					 }else{
